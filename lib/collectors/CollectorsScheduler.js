...
class CollectorsScheduler {
    ...
    setRateLimitMultiplier(multiplier) {
        const isAcceptableMultiplier = this.checkMultiplier(multiplier);

        if (isAcceptableMultiplier) {
            this.rateLimitMultiplier = multiplier;
        }

        return isAcceptableMultiplier;
    }

    checkMultiplier(multiplier) {
        if (!multiplier || multiplier <= 0) return false;
        ...
        return intervalInMilliseconds < MILLISECONDS_IN_AN_HOUR;
    }
}
