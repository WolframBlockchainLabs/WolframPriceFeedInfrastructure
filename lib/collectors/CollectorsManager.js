...
class CollectorsManager extends CollectorsScheduler {
    ...
    async setupReplicaChannel() {
        this.amqpClient.getChannel().addSetup(async (channel) => {
            ...
            await channel.bindQueue(queueAssertion.queue, this.exchange, '');

            await channel.consume(
                queueAssertion.queue,
    ...
    async handleLocalRateLimitUpdate() {
        if (!this.checkMultiplier(this.rateLimitMultiplier + 1)) return;

        await this.reload(this.rateLimitMultiplier + 1);
