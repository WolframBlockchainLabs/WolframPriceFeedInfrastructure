...
class BaseScheduler {
    ...
    async start({ operations, rateLimitMultiplier, taskName }) {
    ...
    async reload({ newRateLimitMultiplier, shouldSleep }) {
    ...
    async updateRateLimitMultiplier({ newRateLimitMultiplier, shouldSleep }) {
        const updateMutexRelease = await this.updateRTMMutex.acquire();

        try {
            if (this.validateMultiplier(newRateLimitMultiplier)) {
                await this.reload({ newRateLimitMultiplier, shouldSleep });
            }
        } finally {
            updateMutexRelease();
        }
