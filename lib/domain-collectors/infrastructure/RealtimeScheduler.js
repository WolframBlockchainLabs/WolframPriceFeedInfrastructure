...
class RealtimeScheduler {
    ...
    async updateRateLimitMultiplier() {
        this.updateRTMMutex.acquire();

        const newMultiplier = this.getMultiplierBackoff();
        if (!this.validateMultiplier(newMultiplier)) return;

        await this.reload(newMultiplier);

        this.updateRTMMutex.release();
    }
