...
class ReplicaDiscoveryPolicy extends BaseAMQPPolicy {
    ...
    constructor({ rabbitGroupName, replicaDiscovery, ...options }) {
        ...
        this.currentStatus = {};
        this.currentProtocolStatus = ReplicaDiscoveryPolicy.STATUS.INITIALIZING;
    ...
    scheduleGreeting(delay) {
        setTimeout(this.handleGreeting.bind(this), delay);
    }

    scheduleCronTask(interval) {
        this.cronTask = Cron(interval, this.handleGreeting.bind(this));
    }
    ...
    async handleGreeting() {
        ...
        this.setProtocolStatus(ReplicaDiscoveryPolicy.STATUS.DEBATING);
    }
    ...
    async handleStatusUpdate() {
        ...
        this.setProtocolStatus(ReplicaDiscoveryPolicy.STATUS.RUNNING);
    ...
    shouldCallReloadHandler(data) {
        const isRunning =
            ...
            ReplicaDiscoveryPolicy.STATUS.RUNNING;

        const statusChanged =
            JSON.stringify(data.status) !== JSON.stringify(this.currentStatus);

        return isRunning && statusChanged;
    }
