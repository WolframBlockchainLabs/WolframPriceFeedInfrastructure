...
class ReplicaDiscoveryPolicy extends BaseAMQPPolicy {
    ...
    async handleShareMessage(data) {
        ...
        try {
            const replicaState = this.validateReplicaState(data.status);
    ...
    validateReplicaState(replicaState) {
        const targetStatus = this.amqpManagementTarget.getStatusHandler();

        const isValidRateLimit =
            replicaState.rateLimitMultiplier > targetStatus.rateLimitMultiplier;

        return {
            replicaMembers: replicaState.replicaMembers,
            ...(isValidRateLimit && {
                rateLimitMultiplier: replicaState.rateLimitMultiplier,
            }),
        };
    }

    shouldCallReloadHandler(replicaState) {
        const targetStatus = this.amqpManagementTarget.getStatusHandler();

        const replicaMembersChanged =
            JSON.stringify(replicaState.replicaMembers) !==
