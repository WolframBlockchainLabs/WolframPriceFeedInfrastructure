...
class MarketsManager {
    ...
    async start() {
        ...
        await this.backoffPolicy.start(this.backoffReload.bind(this));
        await this.statusUpdatePolicy.start({
            updateHandler: this.backoffReload.bind(this),
    ...
    async backoffReload({ rateLimitMultiplier }) {
    ...
    async broadcastRateLimitChange(rateLimitMultiplier) {
        await this.backoffReload({ rateLimitMultiplier });
