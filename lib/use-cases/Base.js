...
const { ACTION_FORBIDDEN } = ERROR_CODES;
...
export default class UseCaseBase extends ChistaUseCaseBase {
    ...
    async profile() {
        if (this.context.userId && !this.#profile) {
            this.#profile = await User.findByPk(this.context.userId, {
    ...
    async checkPermissions() {
        const servicePermission = this.constructor.PERMISSION;

        if (servicePermission) {
            const admin = await this.profile();
            const permissions = admin ? await admin.getPermissions() : [];

            if (!permissions.includes(servicePermission)) {
                throwError(ACTION_FORBIDDEN);
            }
        }
    }
