...
export class CandleStickCollector extends Collector {
    async fetchData() {
        ...
        try {
            ...
            logger(level).error(`Candle stick did not receive from ${exchangeAPI.name} for ${symbol} market`, error.message);
        }
    }
    ...
    async saveData(charts, marketId) {
        ...
        try {
            ...
            logger(level).info('Data for CandleStick have saved successfully');
        } catch (error) {
            logger(level).error(`CandleStick did not save for ${exchange} and ${symbol} market`, error.message);
        }
    }
}
