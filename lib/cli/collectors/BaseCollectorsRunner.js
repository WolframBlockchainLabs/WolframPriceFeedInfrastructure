...
import CryptoExchangeRepository from '#domain-collectors/infrastructure/repositories/exchange/CryptoExchangeRepository.js';
...
import { MARKET_MANAGER_IDENTITY_MODIFIERS_DICT } from '#constants/collectors/market-manager-identity-modifiers.js';
...
class BaseCollectorsRunner extends BaseCLIRunner {
    ...
    async stop() {
        this.logger.info({
            message: 'Shutting down collectors',
            collectors: this.marketsAMQPManagers.map((marketsAMQPManager) =>
                marketsAMQPManager.getLogContext(),
            ),
