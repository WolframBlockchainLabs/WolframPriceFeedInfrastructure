...
import  { CollectorManager }     from '../../lib/collector/CollectorManager.js';
...
test.beforeEach(() => {
    ...
    candleStickCollectorStub = sandbox.stub(CandleStickCollector.prototype, 'start');
...
test.skip('Collector Manager process method', async (t) => {
    ...
    const manager = new CollectorManager({ db, marketService: { getMarketInfo: getMarketInfoStub } });

    await manager.process(exchange, symbols);


    t.is(undefined, sinon.assert.calledOnceWithExactly(getMarketInfoStub, exchange, symbols[0]));
