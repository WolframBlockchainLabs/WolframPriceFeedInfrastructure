...
export default [
    {
        ...
        before : async ({ factory }) => {
            const { exchangeName, symbol, newTicker } = await factory.createTicker();

            return { exchangeName, symbol, newTicker };
        },
        test : async ({ t, coreAPI, exchangeName, symbol, newTicker }) => {
            const res = await coreAPI.get(`/exchanges/${exchangeName}/markets/${symbol}/tickers`);
    ...
    },
    {
        ...
        test : async ({ t, coreAPI, symbol }) => {
            const res = await coreAPI.get(`/exchanges/test/markets/${symbol}/tickers`);
    ...
    },
    {
        ...
        test : async ({ t, coreAPI, exchangeName }) => {
            const res = await coreAPI.get(`/exchanges/${exchangeName}/markets/test/tickers`);
    ...
    },
    {
        ...
        test : async ({ t, coreAPI, exchangeName, symbol }) => {
            const res = await coreAPI.get(`/exchanges/${exchangeName}/markets/${symbol}/tickers?rangeDateStart=2023-06-05%2001:38:37&rangeDateEnd=qq`);
    ...
    },
    {
        ...
        test : async ({ t, coreAPI, exchangeName, symbol }) => {
            const res = await coreAPI.get(`/exchanges/${exchangeName}/markets/${symbol}/tickers?rangeDateStart=2023-06-05%2001:38:37&rangeDateEnd=2023-06-04%2001:38:37`);
